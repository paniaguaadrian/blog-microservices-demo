...
app.post('/events', (req, res) => {
  ...
  const moderationBuildURL = `${moderationURL}/events`;

  // Anything that comes to this service as req.body, will be called as event
  const event = req.body;
  console.log('🚀 ~ app.post ~ event:', event);

  // send the event back to the services posts, comments, query and moderation
  ...
  axios.post(moderationBuildURL, event).catch((err) => {
    console.log(err.message);
  });
